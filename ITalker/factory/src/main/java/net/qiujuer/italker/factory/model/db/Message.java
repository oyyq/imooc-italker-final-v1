...
public class Message
    ...
    @Override
    public boolean isSame(GetPushedImpl oldT) {
        // 两个类，是否指向的是同一个消息
        return super.isSame(oldT) && Objects.equals(id, oldT.getId());
    }

    @Override
    public boolean isUiContentSame(GetPushedImpl oldT) {
        // 对于同一个消息当中的字段是否有不同
        // 这里，对于消息，本身消息不可进行修改；只能添加删除
        // 唯一会变化的就是本地（手机端）消息的状态会改变
        return oldT == this || ( status == oldT.getStatus()
                && Objects.equals(content, oldT.getContent()));
