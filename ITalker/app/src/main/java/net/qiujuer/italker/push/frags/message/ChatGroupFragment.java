...
public class ChatGroupFragment extends ChatFragment<Group>
    ...
    private boolean isAdmin = false;            //"我"是否是该群的Admin, 从"不是"到被修改成"是", 要先退出MessageActivity才刷新权限
    ...
    protected ChatContract.Presenter initPresenter() {
        ...
        PushedDataSource repo = null;
        Map<String, DbHelper.ChangedListener> ListenersMap = DbHelper.getChangedListeners()
                .get(GetPushedImpl.class);
        if(ListenersMap != null){
            repo = (PushedDataSource) ListenersMap.get(mReceiverId);
        }

        ChatGroupPresenter presenter = new ChatGroupPresenter(repo, this, mReceiverId);
        setLoaderListener(presenter);
