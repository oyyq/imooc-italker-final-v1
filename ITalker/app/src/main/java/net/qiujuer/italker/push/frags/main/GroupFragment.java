...
public class GroupFragment
        extends PresenterFragment<GroupsContract.Presenter>
    ...
    protected void initWidget(View root) {
        ...
        //mRecycler.setLayoutManager(....)指定RecyclerView对所有项的排列布局
    ...
    public void onStart() {
        ...
        if(mReloadLocalData) {
            //查询所有本地存储的Session, 退出后重新登陆, 或第一次安装apk, 初始刷新
            mPresenter.start();
            mReloadLocalData = false;
        }else {
            //把repository中的应用缓存数据拿出来刷新到界面上
            ((GroupsPresenter)this.mPresenter).reload();
        }
    }
